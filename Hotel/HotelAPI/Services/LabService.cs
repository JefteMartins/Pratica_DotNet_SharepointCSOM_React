...
public class LabService : BaseSharePointService, ILabService
{
    ...
    public async Task<List<object>> SearchTasksAsync(SearchFilters filters)
    {
        ...
        {
            var safeTitle = filters.Title.Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;");
            conditions.Add($"<Contains><FieldRef Name='Title'/><Value Type='Text'>{safeTitle}</Value></Contains>");
        }
        ...
        {
            var dateStr = filters.MinDate.Value.ToString("yyyy-MM-ddTHH:mm:ssZ");
            conditions.Add($"<Geq><FieldRef Name='DueDate'/><Value Type='DateTime' IncludeTimeValue='FALSE'>{dateStr}</Value></Geq>");
        }

        if (filters.MaxDate.HasValue)
        {
            var dateStr = filters.MaxDate.Value.ToString("yyyy-MM-ddTHH:mm:ssZ");
            conditions.Add($"<Leq><FieldRef Name='DueDate'/><Value Type='DateTime' IncludeTimeValue='FALSE'>{dateStr}</Value></Leq>");
        }
        ...
        string queryXml = string.IsNullOrEmpty(camlWhere) ? "" : $"<Query>{camlWhere}<OrderBy><FieldRef Name='ID' Ascending='FALSE'/></OrderBy></Query>";
        var query = new CamlQuery { ViewXml = $"<View>{queryXml}<RowLimit>50</RowLimit></View>" };
        ...
        catch (ServerException ex) when (ex.Message.Contains("limite") || ex.Message.Contains("excede") || ex.Message.Contains("threshold"))
        {
            ...
            context.Load(fallbackItems, collection => collection.Include(
                item => item.Id,
                item => item["Title"],
                item => item["Status"],
                item => item["DueDate"]
            ...
            var allRecentItems = fallbackItems.AsEnumerable().Select(i => new
            {
                Id = i.Id,
                Title = i["Title"]?.ToString() ?? string.Empty,
                Status = i["Status"]?.ToString() ?? string.Empty,
            ...
            if (!string.IsNullOrEmpty(filters.Status))
                allRecentItems = allRecentItems.Where(x => x.Status.Equals(filters.Status, StringComparison.OrdinalIgnoreCase));

            return allRecentItems.Take(50).Cast<object>().ToList();
        }
    }
    ...
    public async Task<List<object>> GetFieldMappingsAsync()
    {
        ...
        context.Load(fields, fs => fs.Include(f => f.Title, f => f.InternalName, f => f.TypeAsString, f => f.Hidden));
        await context.ExecuteQueryRetryAsync();
